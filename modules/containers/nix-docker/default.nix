{ src ? ./srssscs/2020-09-11.nix, nixpkgs ? "<nixpkgs>", system /*? builtins.currentSystem */, pkgs, self
, lib
, stdenv
, fetchurl
, testVersion
...
, }:
let
pkg = builtins.trace (">>>system= "+ system) nixpkgs.legacyPackages.${system};
inherit (pkgs) dockerTools stdenv buildEnv writeText;
  ...
  pkgs = import unstable { inherit system; /*= "x86_64-linux"; */};

  native = builtins.trace (">>>" + nixpkg)
  
    import nixpkg { inherit system; };
...
#     inherit env run image contents path unstable;  
    ...
    name1 = "hello-2.9";
    src = fetchurl {
      url = "mirror://gnu/hello/${name1}.tar.gz";
      sha256 = "sha256-7Lt6IhQZbFf/k0CqcUWOFVmr049tjRaWZoRpNd8ZHqc=";
    };
    # src = null;
    ...
    shellHooks = ''
      ...
      touch Dockerfile
      touch latest/Dockerfile
      touch ssh/Dockerfile
    '';
#       cp -f ${baseDocker} Dockerfile
#       cp -f ${latestDocker} latest/Dockerfile
#       cp -f ${sshDocker} ssh/Dockerfile
#     '';
  ...
  #  hello.overrideAttrs (drv: { REBUILD = builtins.currentTime; })

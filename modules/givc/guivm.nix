  ...
  config = mkIf (cfg.enable && config.ghaf.givc.enable) {
    ...
    systemd.services.dbus-proxy-networkmanager = {
      ...
      serviceConfig = {
        ...
        ExecStart = "${lib.getExe pkgs.dbus-proxy} --source-bus-name org.freedesktop.NetworkManager --source-object-path /org/freedesktop/NetworkManager --proxy-bus-name org.freedesktop.NetworkManager --source-bus-type session --target-bus-type system --nm-mode";
      };
